...
export const INSTRUCTION = [
  ...
  '',
  '```bash',
  'npx vibeward@latest <url> --json --stdout --yes',
  '```',
  '',
  '`--stdout` puts the JSON payload on stdout and every human-facing line on stderr, so',
  'stdout parses cleanly. `--yes` skips the authorization prompt, so only run this against',
  'a site the user owns or was hired to audit.',
  '',
